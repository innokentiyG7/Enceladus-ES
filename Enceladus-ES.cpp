...
*/
//XOR
string genpassXOR(int n) {
    const char alphanum[] = "1234567890+-/*!&$#?=@<>abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    int string_length = sizeof(alphanum) - 1;
    int length = n;
    srand(time(0));
    string a = "";
    for (int i = 0; i < n; i++) {
        a += alphanum[rand() % string_length];
    }
    return a;
}
string xor_cipher(string data, string key, int n) {
    string out = data;
    for (int i = 0; i < n; ++i) {
        out[i] = data[i] ^ key[i % n];
    }
    return out;
}
...
uint32_t F(uint32_t block32b) {
    uint8_t blocks8b[4];
    split_32bits_to_8bits(block32b, blocks8b);
    block32b = Sbox[0][blocks8b[0]];
    block32b += Sbox[1][blocks8b[1]];
    block32b ^= Sbox[2][blocks8b[2]];
    block32b += Sbox[3][blocks8b[3]];
    return block32b;
}
...
void key_extension(uint32_t* keys32b, uint8_t* keyNb, uint16_t len_bits) {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
        for (uint16_t j = 0; j < 256; j += 2) {
            blowfish(init_blocks8b, 'E', keys32b, init_blocks8b, 8);
            Sbox[i][j] = join_8bits_to_32bits(init_blocks8b);
            Sbox[i][j + 1] = join_8bits_to_32bits(init_blocks8b + 4);
        }
    }
}
...
void print_text_dec(uint8_t* array, int a) {
    int n = a;
    string text = "";
    for (int i = 0; i < n; i++) {
        text += "A";
...
void print_bits(uint64_t x, register uint64_t Nbit) {
    for (Nbit = (uint64_t)1 << (Nbit - 1); Nbit > 0x00; Nbit >>= 1)
        printf("%d", (x & Nbit) ? 1 : 0);
    putchar('\n');
}
size_t input_string(uint8_t* buffer) {
    size_t position = 0;
    uint8_t ch;
...
size_t dec_input_string(uint8_t* buffer, int n) {
...
int main() {
    int choice, choice2;
    cout << "Welcome to the Enceladus-ES (Encryption Software) v1.4 \n";
    cout << "Available encryption methods: \n";
    cout << "(1) XOR encryption (for one time messages) \n";
    cout << "(2) Blowfish encryption \n";
    cout << "Choice method of enryption: ";
    cin >> choice;
    if (choice == 1) {
        //XOR cipher
        cout << "\n(1) Encode\n(2) Decode \n: ";
        ...
        if (choice2 == 1) {
            cout << "Encoding\n";
            char choice3;
            cout << "Do you have generated keys? Y/n: ";
            cin >> choice3;
            if ((choice3 == 'Y') || (choice3 == 'y')) {
                uint8_t encrypted[BUFF_SIZE], decrypted[BUFF_SIZE], buffer[BUFF_SIZE];
                uint8_t key64b[56] = "";
                string inputKey;
                cout << "key: ";
                cin >> inputKey;
